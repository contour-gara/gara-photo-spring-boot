...
interface TokenMapper {
    @Insert(
        """
            INSERT INTO token (id, access_token, refresh_token, date_time)
            VALUES (1, #{accessToken}, #{refreshToken}, #{dateTime})
        """
    )
    ...
    @Update(
        """
            UPDATE token
            SET access_token = #{accessToken}, refresh_token = #{refreshToken}, date_time = #{dateTime}
            WHERE id = 1
        """
    )
