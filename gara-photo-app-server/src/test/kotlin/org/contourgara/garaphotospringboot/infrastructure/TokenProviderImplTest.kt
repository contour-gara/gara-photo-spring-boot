...
class TokenProviderImplTest {
  ...
  fun `認可 URL が生成できる`() {
    // execute
    val actual = sut.createUrl(AuthorizationSetting("client-id-dummy", "http://localhost/dummy", listOf("dummy", "dummy"), "challenge"))
  ...
  fun `アクセストークンを取得できる場合、トークンを取得できる`() {
    ...
    // assert
    val expected = Token("access-token", "refresh-token", "client-id", ZonedDateTime.of(LocalDateTime.of(2024, 4, 23, 0, 0, 0), ZoneId.systemDefault()))
    assertThat(actual).isEqualTo(expected)
  }
  ...
  fun `アクセストークンが取得できない場合、例外が投げられる`() {
    ...
    // execute & assert
    assertThatThrownBy { sut.fetchToken(Authorization("client-id", "redirect-uri", "code", "code-challenge")) }.isInstanceOf(TwitterException::class.java)
  }
  ...
  fun `リフレッショントークンからアクセストークンを取得できる場合、トークンを取得できる`() {
    ...
    // execute
    val actual = sut.fetchTokenByRefreshToken(Token("access-token", "refresh-token", "client-id", ZonedDateTime.of(LocalDateTime.of(2024, 4, 20, 0, 0, 0), ZoneId.systemDefault())))

    // assert
    val expected = Token("access-token2", "refresh-token2", "client-id", ZonedDateTime.of(LocalDateTime.of(2024, 4, 23, 0, 0, 0), ZoneId.systemDefault()))
    assertThat(actual).isEqualTo(expected)
  }
  ...
  fun `リフレッショントークンからアクセストークンを取得できない場合、例外が投げられる`() {
    ...
    // execute & assert
    assertThatThrownBy {
      sut.fetchTokenByRefreshToken(Token("access-token", "refresh-token", "client-id", ZonedDateTime.of(LocalDateTime.of(2024, 4, 20, 0, 0, 0), ZoneId.systemDefault())))
